...
export default function ManagePostListBlock() {
  ...
  const postClickHandler = (e) => {
    ...
    if (e.target.type !== 'submit') navigate(`/post/${postId}?manage=true&prev=block`);
  };
  ...
  useEffect(() => {
    postAPI
      .postGetAll()
      .then((res) => setPostList([...res.data?.filter((post) => post.block !== 'NORMAL')]?.reverse()))
