...
export default function ManagePostListEcho() {
  ...
  const postClickHandler = (e) => {
    ...
    if (e.target.type !== 'submit') navigate(`/post/${postId}?manage=true&prev=echo`);
  };
  ...
  useEffect(() => {
    postAPI
      .postGetAll()
      .then((res) => setPostList([...res.data?.filter((post) => post.echo)]?.reverse()))
